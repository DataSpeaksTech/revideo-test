  ...
  if(jobType == "partialRender"){
    ...
    try {
      ...
      const { audioFile, videoFile } = await renderPartialVideo({
        ...
        variables: variables,
        settings: { logProgress: true, viteBasePort: 5000, outDir: "/tmp/", viteConfig: { cacheDir: "/tmp/.vite"}, puppeteer: { args: chromium.args, headless: chromium.headless, executablePath: await chromium.executablePath() }}
      });  
    ...
    try {
      ...
      console.log("concatenated media...");
