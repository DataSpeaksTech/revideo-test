...
function* question2(view: View2D,videoJson : any) {
  ...
  // const fullText = jsonData.json_response.video.scenes[2].question;
  // const duration = 2;
  ...
  yield* all(
    everythingLayoutRef1().position.x(0, 0.25),
    everythingLayoutRef1().position.y(200, 0.25)
  );

  yield* all(
    chatBubbleRef().opacity(1, 0.25),
    rectRef().opacity(1, 0.25),
    chatTxtRef().text(jsonData.json_response.video.scenes[2].question, 3)
  );

  QuestionFarmerVideoRef().pause();

  yield* everythingLayoutRef2().opacity(0, 0.25);
}
