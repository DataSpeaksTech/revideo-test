  ...
  function* machines(view: View2D,videoJson : any) {
    ...
    yield view.add(
      <Layout ref={everythingLayoutRef2}>
        ...
        >
          <Txt
            text={`₹ ${jsonData.json_response.video.scenes[4].variable_value.numeric_value.total_machines_cost} lakhs`}
            fontSize={50}
            fontWeight={500}
            letterSpacing={3}
            fill={"white"}
    ...
    const referenceSentence = "You'll need various machines like lathes, shapers, grinders, and drilling machines, totaling Rs. 5.32 lakhs.";
    const referenceWordCount = referenceSentence.split(' ').length;
  
    // Get the sentence from JSON and calculate its word count
    const sentence = jsonData.json_response.video.scenes[4].sentence;
