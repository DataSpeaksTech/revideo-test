  ...
  function* additionals(view: View2D,videoJson : any) {
    ...
    yield view.add(
      <Layout ref={everythingLayoutRef2}>
        ...
        >
          <Txt
            text={`₹${jsonData.json_response.video.scenes[5].variable_value.numeric_value.total_machine_cost} lakhs`}
            fontSize={50}
            fontWeight={500}
            letterSpacing={3}
            fill={"white"}
    ...
    yield* costRectRef().opacity(1, 0.3);
  
    yield* waitFor(0);
  
    yield* everythingLayoutRef2().opacity(0, 0.3);
  }
