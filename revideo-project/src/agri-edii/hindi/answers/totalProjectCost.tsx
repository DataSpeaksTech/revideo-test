...
function* totalProjectCost(view: View2D,videoJson : any) {
  ...
  yield view.add(
    <Layout ref={everythingLayoutRef2}>
      ...
      >
        <Txt
          ref={moneyTxtRef}
          text={`₹ ${jsonData.json_response.video.scenes[2].variable_value.numeric_value.total_project_cost} लाख`}
          fontSize={60}
          fontWeight={500}
          letterSpacing={4}
          fill={"white"}
  ...
  yield* all(rectRef3().opacity(1, 1), rectRef3().x(400, 1.2));
  yield* all(rectRef4().opacity(1, 1), rectRef4().x(400, 1.2));
  yield* all(rectRef5().opacity(1, 1), rectRef5().x(400, 1.2));
